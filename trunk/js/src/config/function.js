define(function(require, exports, module) {
    ...
    var show_btn = function() {
        ...
        for (var i = 0; i < powerList.length; i++) {
            var cookieItem = powerList[i].getAttribute('data-link') == '/' ? powerList[i].getAttribute('data-link') : '/' + powerList[i].getAttribute('data-link');
            var cookieList = cookie(cookieItem);
            if (cookieList == (cookieItem)) {
                $('[data-link="' + cookieItem + '"]').removeClass('hidden');
            } else {
                $('[data-link="' + cookieItem + '"]').remove();
            }
        }
    }
    ...
    var get_search_data = function(url, rows, callbackFun) {
        $(document).on('click', '.search-button', function() {
            search_data(url, rows, callbackFun);
        })
    }
    ...
    var search_data = function(url, rows, callbackFun) {
        var searchField = $('.chosen-select').val();
        var searchString = $('input.search-control').val();
        var getData = {
            page: 1,
            rows: rows,
        ...
        $.ajax({
            ...
            success: function(returnData) {
                if (returnData.records && returnData.records > getData.rows) {
                    ...
                    search_data(url, dataRows, callbackFun);
                    return false;
                } else {
                    if ($.isFunction(callbackFun)) {
                        change_page(callbackFun(returnData));
                    }
                }
            }
        })
    ...
    exports.show_chosen = show_chosen;

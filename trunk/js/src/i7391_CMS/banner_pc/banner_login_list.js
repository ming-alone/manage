define(function(require, exports, module) {
    ...
    /*獲取登入頁banner列表 */
    function get_banner_list(rows, callbackFun) {
        var rows = rows || 0;
        var getData = {
            page: 1,
            rows: rows
    ...
    var btn_banner_add_modal = function() {
        $(document).on('click', '[data-link="/i7391_CMS/banner_pc/banner_login_add"]', function() {
            banner_add_modal();
        })
    }
    ...
    var ajax_get_banner_delete_all = function() {
        $(document).on('click', '[data-link="/i7391_CMS/banner_pc/banner_login_delete"]:not(".del")', function(e) {
    ...
    var get_search_data = function () {
        $(document).on('click', '.search-button', function () {
            siteFun.search_data(siteCfg.REQUEST_URL.BANNER_LOGIN_LIST, 1000, get_table_data);
        })
    }

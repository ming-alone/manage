define(function (require, exports, module) {
    ...
    var ajax_get_goods_pc_list = function () {
        get_goods_pc_list(table_render);
    }

    /*獲取推薦商品列表 */
    var get_goods_pc_list = function (callbackFun) {
        $.ajax({
            type: 'GET',
    ...
    var ajax_get_goods_pc_delete_all = function () {
        $(document).on('click', 'a[href="i7391_CMS/goods_pc/goods_delete"]', function (e) {
    ...
    var goods_pc_delete_all_modal = function () {
        var html = '<h4>確認要刪除所選的推薦商品嗎?</h4>' +
            '<hr/>' +
            '<button type="button" data-dismiss="modal" id="continue" class="btn btn-block btn-primary">确定</button>' +
            '<button type="button" data-dismiss="modal" id="cancel" class="btn btn-block btn-primary">取消</button>';
        var myModalTrigger = new $.zui.ModalTrigger({
            width: '300px',
            height: 'auto',

define(function (require, exports, module) {
    ...
    var ajax_get_footer_list = function () {
        get_footer_list(1000, table_render);
    }

    /*獲取登入頁底部廣告列表 */
    var get_footer_list = function (rows, callbackFun) {
        var rows = rows || 0;
        var getData = {
            page: 1,
            rows: rows
        ...
        $.ajax({
            ...
            success: function (returnData) {
                if (returnData.records && returnData.records > getData.rows) {
                    ...
                    get_footer_list(dataRows, callbackFun);
                    return false;
                }
        ...
        for (var j = 0; j < data.length; j++) {
            var rows = {
                ...
                option: '<div class="btn-group">' +
                    '<a data-id=' + data[j].cell.Id + ' href="/i7391_CMS/banner_pc/banner_login_footer_edit" class="power btn edit">編輯</a>' +
                    '<a data-id=' + data[j].cell.Id + ' href="/i7391_CMS/banner_pc/banner_login_footer_delete" class="power btn del">刪除</a>' +
                    '</div>'
            }
            gridList.push(rows);
        }
    ...
    var btn_footer_add_modal = function () {
        $(document).on('click', '[data-link="/i7391_CMS/banner_pc/banner_login_footer_add"]', function () {
            footer_add_modal();
        })
    }
    ...
    var ajax_get_footer_add = function () {
        $(document).on('click', '#footerAddButton', function () {
            $('#footerAddButton').addClass('disabled');
        ...
        $.ajax({
            ...
            success: function (returnData) {
                if (returnData.status === true) {
                    ...
                    $('#footerAddButton').removeClass('disabled');
                }
            }
        })
    }
    ...
    var ajax_post_footer_edit = function () {
        $(document).on('click', '#footerEditButton', function () {
            $('#footerEditButton').addClass('disabled');
            var bannerId = $(this).attr('data-id');
        ...
        $.ajax({
            ...
            success: function (returnData) {
                if (returnData.status === true) {
                    ...
                    $('#footerEditButton').removeClass('disabled');
                }
            }
        })
    }
    ...
    var ajax_get_footer_delete_all = function () {
        $(document).on('click', '[data-link="/i7391_CMS/banner_pc/banner_login_footer_delete"]:not(".del")', function (e) {
    ...
    var get_search_data = function () {
        $(document).on('click', '.search-button', function () {
            siteFun.search_data(siteCfg.REQUEST_URL.BANNER_LOGIN_FOOTER_LIST, 1000, get_table_data);
        })
    }
